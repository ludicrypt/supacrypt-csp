...
Write-Host "=== Supacrypt CSP Full Integration Test Suite ===" -ForegroundColor Green
...
$testSuites = @{
    ...
    "CrossProvider" = @{
        Script = "../scenarios/test_cross_provider_interop.ps1"
        Name = "Cross-Provider Interoperability Tests"
        Timeout = 30
    }
}
...
$masterResults = @{
    ...
    Environment = @{
        ...
        TestRunner = "Supacrypt CSP Integration Suite v1.0"
    }
    SystemMetrics = @{}
}
...
<html>
<head>
    <title>Supacrypt CSP Integration Test Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔐 Supacrypt CSP Integration Test Report</h1>
...
"@
        ...
        if ($suite.SystemMetricsBefore -and $suite.SystemMetricsAfter) {
            $html += @"
                <table class="metrics-table">
                    <tr><th>Metric</th><th>Before</th><th>After</th><th>Change</th></tr>
                    <tr>
                        <td>CPU Usage</td>
                        <td>$($suite.SystemMetricsBefore.CPUUsage)%</td>
                        <td>$($suite.SystemMetricsAfter.CPUUsage)%</td>
                        <td>$(if($suite.SystemMetricsAfter.CPUUsage -ne "N/A") { [math]::Round($suite.SystemMetricsAfter.CPUUsage - $suite.SystemMetricsBefore.CPUUsage, 2) } else { "N/A" })%</td>
                    </tr>
                    <tr>
                        <td>Memory Usage</td>
                        <td>$($suite.SystemMetricsBefore.MemoryUsage)%</td>
                        <td>$($suite.SystemMetricsAfter.MemoryUsage)%</td>
                        <td>$(if($suite.SystemMetricsAfter.MemoryUsage -ne "N/A") { [math]::Round($suite.SystemMetricsAfter.MemoryUsage - $suite.SystemMetricsBefore.MemoryUsage, 2) } else { "N/A" })%</td>
                    </tr>
                </table>
"@
        }
...
foreach ($suiteName in $suitesToRun) {
    if ($testSuites.ContainsKey($suiteName)) {
        ...
        # Update counters (simplified - would need to parse actual test results)
...
if ($ReportFormat -eq "JSON" -or $ReportFormat -eq "Both") {
    $jsonFile = Join-Path $OutputPath "integration_test_report.json"
...
if ($ReportFormat -eq "HTML" -or $ReportFormat -eq "Both") {
    $htmlFile = Join-Path $OutputPath "integration_test_report.html"
...
if ($masterResults.OverallStatus -eq "PASSED") {
    Write-Host "`n🎉 All integration tests completed successfully!" -ForegroundColor Green
    exit 0
} else {
