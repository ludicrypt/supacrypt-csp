# IIS Configuration Script for CSP Integration Testing
...
Write-Host "Configuring IIS for CSP integration testing..." -ForegroundColor Yellow
...
<html>
<head>
    <title>Supacrypt CSP SSL Test</title>
</head>
<body>
    <h1>Supacrypt CSP SSL/TLS Test Page</h1>
...
# Generate test certificate using CSP if no thumbprint provided
if ([string]::IsNullOrEmpty($CertificateThumbprint)) {
    Write-Host "Generating test certificate using Supacrypt CSP..." -ForegroundColor Cyan
    
    $certParams = @{
        Subject = "CN=$SiteName.test.local"
        Provider = "Supacrypt CSP"
    ...
    try {
        ...
        Write-Warning "Failed to generate certificate with CSP: $_"
